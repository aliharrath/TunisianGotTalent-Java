 ...
 * @author Ali
 */
public class ModifierDonation extends BaseForm {
    ...
    ImageViewer iv;
    public ModifierDonation(Form previous, Donation don) {
        ...
        multi.setRows(4);
        multi.setColumns(20);
        multi.setText(don.getDescription());
        input.addComponent(multi);

        setTitle("Update a Donatoin");
        setLayout(BoxLayout.y());

        TextComponent lib = new TextComponent().text(don.getLib_donation()).labelAndHint("Nom du don");
...
//        FontImage.setMaterialIcon(bio.getField().getHintLabel(), FontImage.MATERIAL_COMMENT);
        ...
        TextComponent valeurd = new TextComponent().text(String.valueOf(don.getValeur_d())).labelAndHint("Valeur en diamants").multiline(true);
        ...
        box.setSelectedItem(don.getCategorie());
            ...
            public void actionPerformed(ActionEvent evt) {
                ...
                if ((lib.getText().length() == 0) || (multi.getText().length() == 0) || (valeurd.getText().length() == 0) || (path.getText().length() == 0)  ) {
                    ...
                    try {
                        ...
                        if (ServiceDonation.getInstance().modifierdon(d,don.getId())) {
                            //Dialog.show("Success", "Connection accepted", "OK", null);
                           System.out.println("ffgdfggfdgfdfgd");
                            ToastBar.showMessage("Modification avec succée ! ", FontImage.MATERIAL_CHECK_CIRCLE);
